...
void PspmtProcessor::DeclarePlots(void) {
    histo.DeclareHistogram2D(DD_QDC, SD, S3, "QDC - Dynode 0 - Anodes 1-4");
    histo.DeclareHistogram2D(DD_POSITION_ENERGY, SB, SB, "Pos from Raw Energy");
    histo.DeclareHistogram2D(DD_POSITION_QDC, SB, SB, "Pos from QDC");
    histo.DeclareHistogram2D(DD_POSITION_TRACE, SB, SB, "Pos from TraceFilter");
    histo.DeclareHistogram2D(DD_PIXEL_MAP, S5, S5, "Position by pixel");
    histo.DeclareHistogram2D(DD_PIXEL, SB, SB, "Plot of select pixels");
}
...
bool PspmtProcessor::PreProcess(RawEvent &event) {
    ...
    for(vector<ChanEvent *>::const_iterator it = dynodeEvents.begin();
        ...
        histo.Plot(DD_QDC, (*it)->GetTrace().GetQdc(), 0);
    }
    ...
    histo.Plot(DD_POSITION_ENERGY, posEnergy_.first*histogramScale_+histogramOffset_,
         posEnergy_.second*histogramScale_+histogramOffset_);
    histo.Plot(DD_POSITION_QDC, posQdc_.first*histogramScale_+histogramOffset_,
         posQdc_.second*histogramScale_+histogramOffset_);
    histo.Plot(DD_POSITION_TRACE, posTrace_.first*histogramScale_+histogramOffset_,
         posTrace_.second*histogramScale_+histogramOffset_);
    histo.Plot(DD_PIXEL_MAP, pixel_.first, pixel_.second);

    if (pixel_.first == 12)
        histo.Plot(DD_PIXEL, posEnergy_.first*histogramScale_+histogramOffset_,
	     posEnergy_.second*histogramScale_+histogramOffset_);
