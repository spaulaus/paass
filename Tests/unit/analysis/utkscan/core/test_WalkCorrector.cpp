...
TEST_SUITE ("Analysis/Utkscan/Core/WalkCorrector") {
    TEST_CASE_FIXTURE (WalkCorrector, "Model_None") {
        CHECK (Model_None() == 0.0);
    }

    TEST_CASE_FIXTURE (WalkCorrector, "ModelA") {
    ...
    TEST_CASE_FIXTURE (WalkCorrector, "Model_B1") {
    ...
    TEST_CASE_FIXTURE (WalkCorrector, "Model_B2") {
    ...
    TEST_CASE_FIXTURE (WalkCorrector, "Model_VS_Low") {
        vector<double> par = {0.5, 2.1, 3.7};
        double raw = 20.3;
    ...
    TEST_CASE_FIXTURE (WalkCorrector, "Model_VS_High") {
        vector<double> par = {0.5, 2.1, 3.7};
        double raw = 4000;
        double expected = 0.0;
        double result = Model_VS(par, raw);
    ...
    TEST_CASE_FIXTURE (WalkCorrector, "Model_VS") {
        vector<double> par = {0.5, 2.1, 3.7};
        double raw = 300.;
    ...
    TEST_CASE_FIXTURE (WalkCorrector, "Model_VB") {
        vector<double> par = {0.5, 2.1, 3.7};
        double raw = 300.;
    ...
    TEST_CASE_FIXTURE (WalkCorrector, "Model_VD") {
        vector<double> par = {0.5, 2.1, 3.7};
        double raw = 300.;
        double expected =
        92.7907602830327 * exp(-raw / 186091.225414275) + 0.59140785215161 * exp(raw / 2068.14618331387) -
        95.5388835298589;
        double result = Model_VD(par, raw);
        CHECK(result == expected);
    }

    TEST_CASE_FIXTURE (WalkCorrector, "GetCorrection") {
        ...
        CHECK(expected == GetCorrection(cfg, raw));
    }
}